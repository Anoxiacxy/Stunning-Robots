...
def parse_args():
    parser = argparse.ArgumentParser("MADDPG with Stunning Robots")
...
def main(args):
    ...
    def env_creater():
        _env = stunning_robots.parallel_env(**stunning_robots.DEFAULT_CONFIG, max_steps=500)
        # _env = stunning_robots.aec_env(**stunning_robots.DEFAULT_CONFIG, max_steps=10000)
        return _env

    register_env("env_name", lambda cfg: PettingZooParallelToRlLibMultiAgentEnv(env_creater()))
